...
class Subgraph:
    ...
    def __init__(self, graph):
        ...
        for item in self.g.iterkeys():
            self.g[item].unique_neighbors = len(
                set([x for x in self.g[item].neighbors])
            )
...
class Assembler:
    def __init__(self, data, k=None):
        ...
        if k is None:
            ...
            if clargs.verbose:
                print(
                    "Building de Bruijn graph with",
                    str(clargs.kmer_length) + "-mers... please wait!",
                )
                    ...
                    if len(node.get_parents()) == 1:
                        ...
                        if parent.get_parents():
                            ...
                            if parent.name[1:] == cur.name[: (kmer_len - 1)]:
                                ...
                                newnode.unique_parents = len(
                                    set(self.G[real_parent].parents)
                                )
                                newnode.unique_neighbors = 0
                                newnode.name = conc
                                self.G[real_parent].neighbors = [
                                    x
                                    for x in self.G[real_parent].neighbors
                                    if x != parent
                                ]
            ...
            for k, v in self.G.items():
                ...
                if nbor_list:
                    try:
                        ...
                        if (
                            nbor_list.count(next_node) == len(nbor_list)
                            ...
                            for parent in a.G[n1.name].parents:
                                a.G[parent].neighbors = [
                                    newnode.name if x == n1.name else x
                                    for x in a.G[parent].neighbors
                                ]
    ...
    def trim_branches(self):
        ...
        branch_nodes = [
            ...
            if a.G[x].is_branching() and a.G[x].unique_neighbors > 1
        ]
    ...
    def get_connected_components(self):
        ...
        if clargs.verbose:
            print(
                "Potential number of individual connected components: ", len(head_list)
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "kmer_length", type=int, help="Desired k-mer length (integer value)."
    )
    p.add_argument(
        "-v",
        "--verbose",
        help="Print out some more info about the program at runtime",
        action="store_true",
    )
    p.add_argument(
        "-l", "--logging", help="Write run information to log file", action="store_true"
    )
    p.add_argument(
        "-w",
        "--weighted_dot",
    ...
    if clargs.kmer_length < 3:
        ...
        print("\n")
        print(
            "FATAL: kmer length must be 3 or more! - suggested to use kmer length of 4 at minimum."
        )
        sys.exit(1)
    ...
    if clargs.read_file:
        ...
        if clargs.read_file.next()[0] not in acceptable_start_chars:
            print("\n")
            print(
                "FATAL: unrecognized input file format! - please check your input file."
            )
            sys.exit(1)
    # END ARGPARSE CODE
    ...
    if clargs.verbose:
        ...
        print(
            "\tTotal nodes: ",
            a.unbalanced_nodes + a.balanced_nodes + a.semi_balanced_nodes,
        )
    ...
    if clargs.logging:
        with open("log.csv", "ab") as f:
            writer = csv.writer(f)
            row = (
                clargs.read_file.name,
                str(clargs.kmer_length),
                str(a.unbalanced_nodes),
                str(a.balanced_nodes),
                str(a.semi_balanced_nodes),
                str(len(a.G)),
                str(a.branching_nodes),
                str(a.get_eulerianess()),
                runtime,
            )
            writer.writerow(row)
