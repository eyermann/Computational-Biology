...
class Simulator:
    ...
    def mangle_data(self, sequence):
        ...
        for i in range(
            len(sequence)
        ):  # for nucleotide in sequence: s[0] to s[19] i range 1 to 19
            # roll the dice
            ...
            if error <= error_percent:
                ...
                # handle non-edge cases
                    ...
                    if len(sequence) == self.read_length:
                        sequence = (
                            sequence[:i] + self.mutate(sequence[i]) + sequence[i + 1 :]
                        )
                        self.mutation_count += 1
        return sequence
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Sequence Read Simulator.\r\n Written by Charles Eyermann and Sam Hinh for CS362 Winter 2016"
    )
    parser.add_argument(
        "fasta_sequence_file",
        type=argparse.FileType("r"),
        help="The input file should be in FASTA format",
    )
    parser.add_argument(
        "coverage",
        type=int,
        help="How many times the entire genome should be sequenced",
    )
    parser.add_argument(
        "read_length", type=int, help="The length of each read generated"
    )
