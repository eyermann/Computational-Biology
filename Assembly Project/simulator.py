...
class Simulator:
	...
	def write_file(self, reads):
		...
		with open("reads.txt", "w") as f:
			h = ">" + self.header[1:30] + "| coverage: " + str(self.coverage) + "| read length: " + str(
				self.read_length) + "| error rate: " + str(self.error_rate) + "|\n"
			f.write(h)
...
if __name__ == '__main__':
	...
	parser.add_argument("fasta_sequence_file", type=argparse.FileType('r'),
	...
	parser.add_argument('-v', '--verbose', help="Print out some more info about the program at runtime",
						action="store_true")
